...
test("the cover counts country aliases as one place", () => {
    const hero = read("src/components/Hero.astro");

    assert.match(hero, /countryAliases/);
    assert.match(hero, /"czech-republic":\s*"czech"/);
    assert.match(hero, /countryAliases\[p\.data\.country\]\s*\?\?\s*p\.data\.country/);
});

test("field notes do not claim one camera setup for every panorama", () => {
    const sources = [
        "src/layouts/Base.astro",
        "src/components/Hero.astro",
        "src/components/PanoramaCard.astro",
        "src/pages/[slug].astro",
    ].map(read).join("\n");

    assert.doesNotMatch(sources, /iPhone X|Osmo M3|DJI Osmo Mobile 3/);
    assert.match(sources, /one slow sweep at a time/);
});
